...
// UPDATED: Fixed timeout issue by making email non-blocking
export const sendDeliveryOtp = async (req, res) => {
  try {
    ...
    // corrected: pass the populated user object
    ...
    return res
      .status(200)
      .json({ message: `otp sent sucessfully to ${order?.user?.fullName}` });
  } catch (error) {
...
// UPDATED: Added socket emission after successful delivery
export const verifyDeliveryOtp = async (req, res) => {
  try {
    ...
    // Emit socket events after successful delivery
    const io = req.app.get("io");
    if (io) {
      // Notify customer
      ...
      // Broadcast to all (for admin dashboard)
